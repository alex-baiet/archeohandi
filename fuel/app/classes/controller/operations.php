		...
		if (Input::post('supp_op')){
			if (is_numeric(Input::post('supp_op'))) {
				...
				if (!empty($if_op_ex)){
					...
					/*$query = DB::query('SELECT id_groupe_sujets FROM groupe_sujets WHERE id_operation='.Input::post('supp_op').' ');
					$all_op = $query->execute();
					$all_op= $all_op->_results;

					foreach($all_op as $key):
					$query = DB::delete('sujet_handicape')->where('id_groupe_sujets', $key['id_groupe_sujets']);
					$sujet_delete= $query->execute();

					$query = DB::delete('groupe_sujets')->where('id_groupe_sujets', $key['id_groupe_sujets']);
					$groupe_delete= $query->execute();
				endforeach;

				$query = DB::delete('operations')->where('id_site', Input::post('supp_op'));
				$op_delete= $query->execute();*/
...
public function action_view($id){
	...
	if (Input::post('supp_sujet')){
		if (is_numeric(Input::post('supp_sujet'))) {
			...
			if (!empty($if_op_ex)){
				/*$query = DB::delete('sujet_handicape')->where('id_sujet_handicape', Input::post('supp_sujet'));
				$sujet_delete= $query->execute();*/
	...
	if (Input::post("modif_op")){
		...
		if(Input::post('commune') != null):
			...
			if(!empty(Input::post('EA'))): 
				if(verif_alpha(Input::post('EA'), 'alphanum') != false): $EA=verif_alpha(Input::post('EA'),'alphanum');  $valeurs.='&EA='.$EA; else: $erreurs.="&erreur_alpha_ea"; endif; 
			endif;
			if(!empty(Input::post('OA'))): 
				if(verif_alpha(Input::post('OA'), 'alphanum') != false): $OA=verif_alpha(Input::post('OA'),'alphanum'); $valeurs.='&OA='.$OA; else: $erreurs.="&erreur_alpha_oa"; endif; 
			endif;
			if(!empty(Input::post('patriarche'))): 
				if(verif_alpha(Input::post('patriarche'), 'alphanum') != false): $patriarche=verif_alpha(Input::post('patriarche'), 'alphanum'); $valeurs.='&patriarche='.$patriarche; else: $erreurs.="&erreur_alpha_patriarche"; endif; 
			endif;
			if(!empty(Input::post('numero_operation'))): 
				if(verif_alpha(Input::post('numero_operation'), 'alphanum') != false): $num_op=verif_alpha(Input::post('numero_operation'), 'alphanum'); $valeurs.='&numero_op='.$num_op; else: $erreurs.="&erreur_alpha_numop"; endif;
			endif;
			...
			if(!empty(Input::post('anthropologue')) ): 
				if(verif_alpha(Input::post('anthropologue'), 'alpha') != false): $anthropologue=verif_alpha(Input::post('anthropologue'), 'alpha'); $valeurs.='&anthropologue='.$anthropologue; else: $erreurs.="&erreur_alpha_anthro"; endif; 
			endif;
			if(!empty(Input::post('paleopathologiste')) ): 
				if(verif_alpha(Input::post('paleopathologiste'), 'alpha') != false): $paleopathologiste=verif_alpha(Input::post('paleopathologiste'), 'alpha'); $valeurs.='&paleopathologiste='.$paleopathologiste; else: $erreurs.="&erreur_alpha_paleo"; endif; 
			endif;
			...
			if ($erreurs!="") { Response::redirect('/operations/edit/operation/'.$id.'?'.$erreurs.''.$valeurs.''); die;}
