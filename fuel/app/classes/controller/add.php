...
function verif_alpha($str,$type){
	...
	strip_tags($str);
   	...
   	if($type == "alpha"): preg_match('/([^A-Za-zàáâãäåçèéêëìíîïðòóôõöùúûüýÿ ])/',$str,$result); endif;

    if($type == "alphatout"): preg_match('/([^A-Za-z0-9àáâãäåçèéêëìíîïðòóôõöùúûüýÿ ])/',$str,$result); endif;

    if($type == "alphanum"): preg_match('/([^A-Za-z0-9,-;()\/ ])/',$str,$result); endif;
...
if(!empty(Input::post('adresse'))): if(verif_alpha(Input::post('adresse'), 'alphatout') != false): $adresse=verif_alpha(Input::post('adresse'), 'alphatout'); $valeurs.='&adresse='.$adresse; else: $erreurs.="&erreur_alpha_adresse"; endif; else: $erreurs.="&erreur_adresse"; endif;
...
if (!empty($if_commune_ex)): $valeurs.='&commune='.Input::post('commune'); $id_commune=$if_commune_ex[0]['id']; else: $erreurs.="&erreur_commune"; endif;
else: $erreurs.="&erreur_commune_select"; endif;
...
if(!empty($if_organisme_ex)): $valeurs.='&organisme='.Input::post('organisme'); $organisme=Input::post('organisme'); else: $erreurs.="&erreur_organisme"; endif;
else: $erreurs.="&erreur_organisme_select"; endif;
...
if(!empty($if_type_operation_ex)): $valeurs.='&type_op='.Input::post('type_operation'); $type_operation=Input::post('type_operation'); else: $erreurs.="&erreur_type_operation"; endif;
else: $erreurs.="&erreur_type_operation_select"; endif;

if(!empty(Input::post('a_revoir'))): $a_revoir=trim(strip_tags(Input::post('a_revoir'))); $valeurs.='&a_revoir='.$a_revoir; endif;
if(!empty(Input::post('EA'))): if(verif_alpha(Input::post('EA'), 'alphanum') != false): $EA=verif_alpha(Input::post('EA'),'alphanum');  $valeurs.='&EA='.$EA; else: $erreurs.="&erreur_alpha_ea"; endif; endif;
if(!empty(Input::post('OA'))): if(verif_alpha(Input::post('OA'), 'alphanum') != false): $OA=verif_alpha(Input::post('OA'),'alphanum'); $valeurs.='&OA='.$OA; else: $erreurs.="&erreur_alpha_oa"; endif; endif;
if(!empty(Input::post('patriarche'))): if(verif_alpha(Input::post('patriarche'), 'alphanum') != false): $patriarche=verif_alpha(Input::post('patriarche'), 'alphanum'); $valeurs.='&patriarche='.$patriarche; else: $erreurs.="&erreur_alpha_patriarche"; endif; endif;
if(!empty(Input::post('numero_operation'))): if(verif_alpha(Input::post('numero_operation'), 'alphanum') != false): $num_op=verif_alpha(Input::post('numero_operation'), 'alphanum'); $valeurs.='&numero_op='.$num_op; else: $erreurs.="&erreur_alpha_numop"; endif; endif;
if(!empty(Input::post('arrete_prescription'))): if(verif_alpha(Input::post('arrete_prescription'), 'alphanum') != false): $prescription=verif_alpha(Input::post('arrete_prescription'), 'alphanum'); $valeurs.='&arrete_prescription='.$prescription; else: $erreurs.="&erreur_alpha_prescription"; endif; endif;
if(!empty(Input::post('responsable_op'))): if(verif_alpha(Input::post('responsable_op'), 'alpha') != false): $RO=verif_alpha(Input::post('responsable_op'), 'alpha'); $valeurs.='&responsable_op='.$RO; else: $erreurs.="&erreur_alpha_ro"; endif; endif;
...
foreach(Input::post('anthropologue') as $key => $val):
	...
	if(array_key_last(Input::post('anthropologue')) == $key): $anthropologue.= $val; else: $anthropologue.= $val.', '; endif; 
	else: $erreurs.="&erreur_alpha_anthro";  endif;
	endif;
endforeach;
...
foreach(Input::post('paleopathologiste') as $key => $val):
	if(!empty($key)):
	if(verif_alpha($val, 'alpha') != false):
		if(array_key_last(Input::post('paleopathologiste')) == $key): $paleopathologiste.= $val; else: $paleopathologiste.= $val.', '; endif;
	else: $erreurs.="&erreur_alpha_paleo"; endif;
	endif;
endforeach;
$valeurs.='&paleopathologiste='.$paleopathologiste;
endif;

if(!empty(Input::post('bibliographie')) ): $bibliographie=trim(strip_tags(Input::post('bibliographie'))); $valeurs.='&bibliographie='.$bibliographie; endif;

//Si erreur est non vide, nous serrons redirigés avec les erreurs et les valeurs dans l'url pour les garder en mémoire
if ($erreurs!="") { Response::redirect('/add/operation?erreur_operation='.$erreurs.''.$valeurs.''); die;}
...
else: Response::redirect('/operations'); endif;
}
...
if(!empty($if_op_ex)): $id_operation=Input::post('id_operation'); $nom_op= Input::post('nom_op'); else: Response::redirect('/add/operation'); endif;
else:
Response::redirect('/add/operation');
endif;
...
for($i=1; $i<=$nb; $i++): $array[$i]=array(); $description_diagnostic_tab[$i]=array(); $commentaire_appareil[$i]=array(); for($a=1; $a<=13; $a++):$localisation[$i][$a]=array(); $appareil[$i][$a]=array();endfor; $description_patho_tab[$i]=array(); endfor;
...
if($val != null):
if(is_numeric($val)): $valeurs.='&NMI_'.$key.'='.$val; $array[$key]+=array('NMI'=>$val); else: $erreurs.="&erreur_NMI_".$key; endif;
else: $erreurs.="&erreur_NMI_vide_".$key; endif;
endforeach;
...
if(!empty($if_chrono_ex)): $valeurs.='&chronologie_'.$key.'='.$val; $array[$key]+=array('chronologie'=>$val); else: $erreurs.="&erreur_chrono_".$key; endif;
else: $erreurs.="&erreur_chrono_vide_".$key; endif;
endforeach;
...
else: $erreurs.="&erreur_alpha_sujet_".$key; endif;
else: $erreurs.="&erreur_sujet_vide_".$key; endif;
endforeach;

foreach(Input::post('sexe') as $key => $val):
if($val != null):
if($val=="Femme" || $val=="Homme" || $val=="Indéterminé" ): $valeurs.='&sexe_'.$key.'='.$val; $array[$key]+=array('sexe'=>$val); else: $erreurs.="&erreur_sexe_".$key; endif;
else: $erreurs.="&erreur_sexe_vide_".$key; endif;
endforeach;

foreach(Input::post('age_min') as $key => $val):
if(!empty($val)):
if(is_numeric($val) && $val> 0 && $val <130): $valeurs.='&age_min_'.$key.'='.$val; $array[$key]+=array('age_min'=>$val); else: $erreurs.="&erreur_age_min_".$key; endif;
else: $array[$key]+=array('age_min'=>""); endif;
endforeach;

foreach(Input::post('age_max') as $key => $val):
if(!empty($val)):
if(is_numeric($val) && $val> 0 && $val <130 && $val > $array[$key]['age_min']): $valeurs.='&age_max_'.$key.'='.$val; $array[$key]+=array('age_max'=>$val); else: $erreurs.="&erreur_age_max_".$key; endif;
else: $array[$key]+=array('age_max'=>""); endif;
endforeach;

foreach(Input::post('datation_debut') as $key => $val):
if(!empty($val)):
if(is_numeric($val)): $valeurs.='&datation_debut_'.$key.'='.$val; $array[$key]+=array('datation_debut'=>$val); else: $erreurs.="&erreur_datation_debut_".$key; endif;
else:  $array[$key]+=array('datation_debut'=>""); endif;
endforeach;

foreach(Input::post('datation_fin') as $key => $val):
if(!empty($val)):
if(is_numeric($val) && $val > $array[$key]['datation_debut']): 	$valeurs.='&datation_fin_'.$key.'='.$val; $array[$key]+=array('datation_fin'=>$val); else: $erreurs.="&erreur_datation_fin_".$key; endif;
else:  $array[$key]+=array('datation_fin'=>$val); endif;
endforeach;
...
if(!empty($if_depot_ex)): $valeurs.='&type_depot_'.$key.'='.$val; $array[$key]+=array('type_depot'=>$val); else: $erreurs.="&erreur_depot_".$key; endif;
else: $erreurs.="&erreur_depot_vide_".$key; endif;
endforeach;
...
if(!empty($if_sepulture_ex)): $valeurs.='&type_sepulture_'.$key.'='.$val; $array[$key]+=array('type_sepulture'=>$val); else: $erreurs.="&erreur_sepulture_".$key; endif;
else: $erreurs.="&erreur_sepulture_vide_".$key; endif;
endforeach;

foreach(Input::post('contexte_normatif') as $key => $val):
if($val != null):
if($val == "Standard" || $val == "Atypique"): $valeurs.='&contexte_normatif_'.$key.'='.$val; $array[$key]+=array('contexte_normatif'=>$val); else: $erreurs.="&erreur_contexte_normatif_".$key; endif;
else: $array[$key]+=array('contexte_normatif'=>""); endif;
endforeach;

foreach(Input::post('milieu_vie') as $key => $val):
if($val != null):
if($val == "Rural" || $val == "Urbain"): $valeurs.='&milieu_vie_'.$key.'='.$val; $array[$key]+=array('milieu_vie'=>$val); else: $erreurs.="&erreur_milieu_vie_".$key; endif;
else: $array[$key]+=array('milieu_vie'=>""); endif;
endforeach;

foreach(Input::post('contexte') as $key => $val):
if($val != null):
if($val == "Funeraire" || $val == "Domestique" || $val == "Autre"): $valeurs.='&contexte_'.$key.'='.$val; $array[$key]+=array('contexte'=>$val); else: $erreurs.="&erreur_contexte_".$key; endif;
else: $array[$key]+=array('contexte'=>""); endif;
endforeach;
...
else: $array[$key]+=array('commentaire_contexte'=>""); endif;
endforeach;


if(Input::post('accessoire_vestimentaire_et_parure') != NULL):
foreach(Input::post('accessoire_vestimentaire_et_parure') as $key => $val):
$valeurs.='&accessoire_vestimentaire_et_parure_'.$key; $array[$key]+=array('accessoire_vestimentaire_et_parure'=>$val);
endforeach;
endif;

if(Input::post('armement_objet_de_prestige') != NULL):   
foreach(Input::post('armement_objet_de_prestige') as $key => $val):
$valeurs.='&armement_objet_de_prestige_'.$key; $array[$key]+=array('armement_objet_de_prestige'=>$val);
endforeach;
endif;

if(Input::post('depot_de_recipient') != NULL):  
foreach(Input::post('depot_de_recipient') as $key => $val):
$valeurs.='&depot_de_recipient_'.$key; $array[$key]+=array('depot_de_recipient'=>$val);
endforeach;
endif;
...
else: $erreurs.="&erreur_description_mobilier_vide_".$key; endif;
endif;          
endforeach;
endif;
...
else: $erreurs.="&erreur_alpha_num_inventaire_".$key; endif;
endif;
endforeach;
endif;
...
//Vérifie quelle existe dans la BDD
if(!empty($if_commune_depot_ex)): $valeurs.='&commune_depot_'.$key.'='.$nom_commune_depot; $array[$key]+=array('commune_depot'=>$if_commune_depot_ex[0]['id']); else: $erreurs.="&erreur_commune_depot_".$key; endif;
else: $array[$key]+=array('commune_depot'=>""); endif;
endforeach;
endif;
...
else: $erreurs.="&erreur_alpha_adresse_depot_".$key; endif;
endif;
endforeach;
endif;
...
//Ajout du nom de la pathologie dans le tableau permettant de construire la phrase qui s'ajoute au commentaire du diagnostic
array_push($description_patho_tab[$key], 'Tuberculose'); endif;
...
//Ajout du nom de la pathologie dans le tableau permettant de construire la phrase qui s'ajoute au commentaire du diagnostic
array_push($description_patho_tab[$key], 'Autre pathologie'); endif;

else: $erreurs.='&erreur_pathologie_'.$key; endif; 
endforeach;
endif;
...
else: $erreurs.="&erreur_description_autre_atteinte_".$key; endif;
endif;          
endforeach;
endif;
...
if(array_key_last($val3) == $key4): $ajout_description_diagnostic.= " ".$val4.'. ';  else: $ajout_description_diagnostic.= $val4.', '; endif;
endforeach;
endforeach;
elseif ($val2 == "Autre atteinte"):
$ajout_description_diagnostic.= $val2.': ';
else: $ajout_description_diagnostic.= $val2.', '; endif;
endforeach;
...
else: $array[$key]+=array('commentaire_diagnostic'=>$ajout_description_diagnostic); endif;
else: $array[$key]+=array('commentaire_diagnostic'=>$ajout_description_diagnostic); endif;
endforeach;
...
if(!empty($key['num_inventaire'])): $num_inventaire=$key['num_inventaire']; else: $num_inventaire=""; endif;
if(!empty($key['commune_depot'])): $commune_depot=$key['commune_depot']; else: $commune_depot=""; endif;
if(!empty($key['adresse_depot'])): $adresse_depot=$key['adresse_depot']; else: $adresse_depot=""; endif;
//Ajout du dépôt
...
if(!empty($val99)): $commentaire=$val99; else: $commentaire=""; endif;
foreach ($val2 as $key3 => $val3) {
$id_diagnostic=$key2;
...
Response::redirect('/add/operation?op='.$id_operation.'&nom='.$nom_op.'&nb='.$nb.'&erreur_sujet='.$erreurs.''.$valeurs.'');
}else{
Response::redirect('/operations');
}
}
//Permet de changer le titre de la fenêtre quand nous ajoutons un sujet
if($erreur_operation == 0): $this->template->title = 'Ajouter des sujets';	else: $this->template->title = 'Ajouter une opération';endif;
$data = array('all_type_op'=> $all_type_op, 'all_organisme' => $all_organisme,'chronologie'=>$chronologie,
...
$this->template->content = View::forge('add/operation', $data);
}
...
$data = array('id'=>$id, 'chronologie'=>$chronologie,'type_depot'=>$type_depot,'type_sepulture'=>$type_sepulture, 'diagnostic'=>$diagnostic,'accessoire'=>$accessoire,'localisation_atteinte'=>$localisation_atteinte, 'pathologie'=>$pathologie,
