...
$query = DB::query('SELECT nom,departement FROM commune WHERE id=' . $operation['id_commune'] . ' ');
$detail_commune = $query->execute();
$detail_commune = $detail_commune->_results[0];

$query = DB::query('SELECT nom FROM organisme WHERE id=' . $operation['id_organisme'] . ' ');
$organisme = $query->execute();
if (isset($organisme->_results[0])) {
  $organisme = $organisme->_results[0]['nom'];
}

$query = DB::query('SELECT nom FROM type_operation WHERE id=' . $operation['id_type_op'] . ' ');
$type_operation = $query->execute();
$type_operation = $type_operation->_results[0]['nom'];
?>
<div class="container">
  <h1 class="m-2">Opération <?= $operation['nom_op']; ?>
    <a class="btn btn-primary btn-sm" href="/public/add/sujet/<?= $operation['id_site']; ?>">Ajouter des sujets
  ...
  <div class="container" style="background-color: #F5F5F5;">
    ...
    <div class="row">
      <div class="col">
        <div class="p-2">Année de l'opération : <?= $operation['annee']; ?></div>
      </div>
      <div class="col">
    ...
    <div class="row">
      ...
      <div class="col">
        <div class="p-2">Adresse : <?= $operation['adresse']; ?></div>
      </div>
    </div>
    <div class="row">
      <div class="col">
        <div class="p-2">Numéro d'opération : <?= $operation['numero_operation']; ?></div>
      </div>
      <div class="col">
        <div class="p-2">Patriarche : <?= $operation['patriarche']; ?></div>
      </div>
      <div class="col">
        <div class="p-2">EA : <?= $operation['EA']; ?></div>
      </div>
    </div>
    <div class="row">
      <div class="col-md-4">
        <div class="p-2">OA : <?= $operation['OA']; ?></div>
      </div>
    </div>
  </div>
  <br />
  <div class="container" style="background-color: #F5F5F5;">
    ...
    <div class="row">
      <div class="col">
        <div class="p-2">Responsable de l'opération : <?= $operation['responsable_op']; ?></div>
      </div>
      <div class="col">
        <div class="p-2">Anthropologue : <?= $operation['anthropologue']; ?></div>
      </div>
      <div class="col">
        <div class="p-2">Paleopathologiste : <?= $operation['paleopathologiste']; ?></div>
      </div>
    </div>
  </div>
  <br />
  <div class="container" style="background-color: #F5F5F5;">
    <h4>Autre</h4>
    <p>Bibliographie : <?= $operation['bibliographie']; ?></p>
  </div>
</div>
<br />
<?php

$query = DB::query('SELECT id_groupe_sujets FROM groupe_sujets WHERE id_operation=' . $operation['id_site'] . ' ');
...
if (!empty($all_sujet_handicap)) : ?>
  <div class="container">
    <div class="row">
      <?php
      $l_sujets_count = 0;
      foreach ($all_sujet_handicap as $operation) :
        foreach ($operation as $operation2) :
          $l_sujets_count++;
        endforeach;
      endforeach;
      ?>
          ...
          <table class="table table-striped table-hover table-bordered sticky" data-toggle="table" data-search="true">
            ...
            <tbody>
              ...
              foreach ($all_sujet_handicap as $operation) :
                ...
                foreach ($operation as $operation2) :
                  $i++;
                  $query = DB::query('SELECT nom FROM type_depot WHERE id=' . $operation2['id_type_depot'] . ' ');
                  $nom_depot = $query->execute();
                  $nom_depot = $nom_depot->_results[0]['nom'];

                  $query = DB::query('SELECT nom FROM type_sepulture WHERE id=' . $operation2['id_sepulture'] . ' ');
                  $nom_sepulture = $query->execute();
                  $nom_sepulture = $nom_sepulture->_results[0]['nom'];
              ?>
                  <tr class="text-center">
                    <?= '<td>' . $operation2['id_sujet_handicape'] . ' (' . $i . ')</td>' ?>
                    <?= '<td>' . $operation2['sexe'] . '</td>' ?>
                    <?= '<td>' . $operation2['datation'] . '</td>' ?>
                    <?= '<td>' . $operation2['milieu_vie'] . '</td>' ?>
                    <?= '<td>' . $nom_depot . '</td>' ?>
                    <?= '<td>' . $nom_sepulture . '</td>' ?>
                    <td class="col-auto">
                      <a title="Consulter #<?= $operation2['id']; ?>" href="/public/sujet/view/<?= $operation2['id']; ?>">
                        <img class="icon see" width="30px" src="https://archeohandi.huma-num.fr/public/assets/img/reply.svg" alt="Consulter">
                      </a>
                      <a title="Editer #<?= $operation2['id']; ?>" href="/public/sujet/edit/<?= $operation2['id']; ?>">
                        <img class="icon edit" width="24px" src="https://archeohandi.huma-num.fr/public/assets/img/pen.svg" alt="Éditer">
                      </a>
                      <form method="post" id="form_suppr_<?= $operation2['id']; ?>">
                        <button type="button" class="btn" name="btn_supp_sujet" value="<?= $operation2['id']; ?>">
                          <img class="icon del" width="25px" src="https://archeohandi.huma-num.fr/public/assets/img/trash.svg" alt="Supprimer">
                          <input type="hidden" name="supp_sujet" value="<?= $operation2['id']; ?>">
                        </button>
                      </form>
                    </td>
                  </tr>
